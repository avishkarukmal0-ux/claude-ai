...
2. **Today so far** — takings (big) + trend **vs the same slice of the day one week ago** (fair like-for-like), sales count, staff on shift. Below it, an **End-of-day summary** CTA → `DaySummaryModal` (`GET /overview/daily-summary`): "Shop closed fine 👍" / "N things to check", takings + profit, a "worth a look" list, and **"Send to my phone"** (`navigator.share`, clipboard fallback) — see [[2026-07-16-Daily-Summary]]. The modal also has an **Auto-delivery** panel: turn on a **nightly WhatsApp** send at the store's closing time (`ownerSummary` on the Store; `jobs/dailySummary.js` every-minute cron; Twilio via `notificationService`, env-gated; `Notification` outbox for idempotency) — see [[2026-07-16-Nightly-WhatsApp-Summary]].
